...
app.controller('userCtrl', ['$scope', '$filter', '$http', '$window', '$location', '$timeout' ,
	function ($scope, $filter, $http, $window, $location, $timeout) {
		...
		$scope.setFlag = function(){			
			...
			$scope.errorUserCountry = 0;
			$scope.errorStateName = 0;
			$scope.errorProject = 0;
			$scope.errorMobileNumber=0;
		}
		...
		$scope.addUser = function() {						
			...
			if(!$scope.firstnameadd) {				
				...
				$scope.errorUserCountry = 1;
				$scope.msgUserCountry="Please select Country";
				document.getElementById("countrynameadd").focus();
			} 
			else if(!$scope.mobilenumberadd){
				$scope.errorMobileNumber = 1;
				$scope.msgMobileNumber="Please select number";
				document.getElementById("mobilenumberadd").focus();
			}else {
				$scope.spin = 1;
				var a = 0;
