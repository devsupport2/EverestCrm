...
app.controller('enquiryCtrl', ['$scope', '$filter', '$http', '$window', '$location', '$timeout' , function ($scope, $filter, $http, $window, $location, $timeout) {
	...
	$scope.next = function(projectid) {
		...
		if($scope.pageSize == "All") {
			...
			$scope.pid = projectid;
			if($scope.pid == null || $scope.pid == "null"){
				projectid = 0;
				$scope.project = undefined;
			}else {
				$scope.project = projectid;
			}
			var link = baseUrl+'getEnquiryListByDateAndProject?pagesize='+$scope.pageSize+'&startindex='+$scope.startindex+'&projectid='+projectid;
			/*var link = baseUrl+'getEnquiriesByPage?pagesize='+$scope.pageSize+'&startindex='+$scope.startindex;*/
			$http.get(link).success(function(data, status, headers, config) {
				$scope.enquiryLoader = false;
				$scope.getEnquiries = data;
	...
	$scope.changePage = function(projectid) {
		$scope.enquiryLoader = true;
		$scope.search = '';
		$scope.currentPage = 0;
		$scope.startindex = 0;
	...
	$scope.searchEnquiry = function(projectid) {
		$scope.enquiryLoader = true;
		if(!$scope.search) {
			if($scope.pageSize == "All") {
				var link = baseUrl+'getAllOpenEnquiries';
	...
	$scope.getAllEnquiryListByName = function(projectid, userID){
		
		$scope.UserAdminCheck = (userID == 1);
