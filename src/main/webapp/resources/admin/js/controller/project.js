...
app.controller('projectCtrl', ['$scope', '$filter', '$http', '$window', '$location', '$timeout' ,
	function ($scope, $filter, $http, $window, $location, $timeout) {
					...
					if($scope.addproject == 'Success') {
						...
						angular.forEach($scope.projectdetaillist,function(item) {
							...
							$http.post(link).success(function(data, status, headers, config) {
								...
								c = c + 1;
								if($scope.banklist.length == h && $scope.specificationlist.length == a && $scope.amenitylist.length == b && $scope.projectdetaillist.length != c && $scope.projectpricedetaillist.length == d && $scope.paymentschedulelist.length == e && $scope.projectarealist.length == f) {
									$scope.spin = 0;
						...
						angular.forEach($scope.banklist,function(item) {
							var link = baseUrl+'addProjectBank?bankid='+item.bankid+'&bankTitle='+item.bankTitle;
							$http.post(link).success(function(data, status, headers, config) {
								$scope.addprojectbank = data;
								c = c + 1;
								if($scope.banklist.length != h && $scope.specificationlist.length == a && $scope.amenitylist.length == b && $scope.projectdetaillist.length == c && $scope.projectpricedetaillist.length == d && $scope.paymentschedulelist.length == e && $scope.projectarealist.length == f) {
									$scope.spin = 0;
									$scope.projectSubmitError = 0;
								...
								$scope.addprojectbank = "Response Fail";
							});
						});
						...
						angular.forEach($scope.projectarealist,function(item) {												
							...
							$http.post(link).success(function(data, status, headers, config) {
								...
								f = f + 1;
								if($scope.banklist.length == h && $scope.projectfloorlayoutlist == g && $scope.specificationlist.length == a && $scope.amenitylist.length == b && $scope.projectdetaillist.length == c && $scope.projectpricedetaillist.length == d && $scope.paymentschedulelist.length == e && $scope.projectarealist.length != f) {
									$scope.spin = 0;
									$scope.projectSubmitError = 0;
									$scope.projectSubmitSuccess = 1;
									$scope.successMsg = "Data added";
		...
		$scope.getProject = function(projectid) {
			...
			$scope.shown_project1 = $scope.projectDetail.shown_project;
				...
				var link = baseUrl+'getAllProjectBankById?projectid='+projectid;
				$http.get(link).success(function(data, status, headers, config) {
					$scope.bankById = data;				 	
				}).error(function(data, status, headers, config) {
					$scope.bankById = "Response Fail";
				});
		...
		$scope.addBankRow = function() {
			...
			if(!$scope.bankadd) {				
				$scope.errorBank = 1;
				$scope.msgBank = "Please select bank!";
				document.getElementById("bankadd").focus();
			} else {				
				for (i in $scope.getBank) {
					if ($scope.getBank[i].bankId == $scope.bankadd) {
						$scope.bankTitle = $scope.getBank[i].bankName;
						break;
					}					
				}
				for (j in $scope.banklist) {
					if ($scope.banklist[j].bankid == $scope.bankadd) {
						$scope.errorBank = 1;
						$scope.msgBank = "This bank is already exist.Please Choose other bank name";	
						return;
					}					
				}
				
				$scope.banklist.push({'bankid' : $scope.bankadd, 'bankTitle' : $scope.bankTitle});
			}
		}
		
		$scope.removeBankRow = function(item) {
			var index = -1;
			for(var i=0; i<$scope.banklist.length; i++) {
				if($scope.banklist[i] == item){
					index = i;
					break;
				}
			}
		...
		$scope.editBankRow = function() {
			
			if(!$scope.bankedit) {
				$scope.errorBank = 1;
				$scope.msgBank = "Please select bank!";
				document.getElementById("bankedit").focus();
			} else {
				for (j in $scope.bankById) {
					if ($scope.bankById[j].bankId == $scope.bankedit) {
						$scope.errorBank = 1;
						$scope.msgBank = "This bank is already exist.Please Choose other bank name";	
						return;
					}
				...
				$scope.spin = 1;
				var link = baseUrl+'editProjectBank?projectid='+$scope.projectid+'&bankid='+$scope.bankedit;
				$http.post(link).success(function(data, status, headers, config) {
					$scope.editbank = data;
					$scope.spin = 0;
					...
					$scope.editbank = data;
					$scope.spin = 0;
					$scope.errorMsg = $scope.editbank;
				});
			}
		}
		
		
		$scope.removeBankRowEdit = function(bankid){
			var link = baseUrl+'deleteProjectBank?projectbankid='+bankid;		
			$http['delete'](link).success(function(data, status, headers, config) {
				$scope.deleteprojectbank = data;
				$scope.getProject($scope.projectid);
			}).error(function(data, status, headers, config) {
				$scope.deleteprojectbank = "Response Fail";
			});
		}
