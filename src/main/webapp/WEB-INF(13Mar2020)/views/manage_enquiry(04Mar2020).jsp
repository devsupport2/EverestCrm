			...
			<div class="content-wrapper">
				<section class="content-header">
					...
					<ol class="breadcrumb">
						...
						<li><button class="btn btn-success" ng-click="exportData()">Export Enquiry</button></li>
									...
									<div class="col-md-3">
										...
										<div class="input-group">
											<select id="clientidadd" name="clientidadd" ng-model="clientidadd" ng-change="setFlag()" class="select2" style="width: 100%;">
												...
												<option ng-repeat="item in getClientNames" value="{{item.userId}}">{{item.firstName}} {{item.lastName}} - {{item.userCompanyName}}</option>
											</select>
											<span class="input-group-btn">
												<button class="btn btn-info btn-flat" type="button" data-toggle="modal" data-target="#userModal" title="Add New Client"><i class="fa fa-plus"></i></button>
											</span>											
							...
							<div class="box box-tgsc">
								...
								<div class="box-header with-border">
									<div class="row">										
										...
										<div class="col-md-2">
											<div class="form-group">
												<select ng-model="project" title="Filter by project" class="form-control" ng-init="changeProjectName('<%= request.getParameter("projectname") %>')">
													<option value="">Project</option>
													<option ng-repeat="item in getProjectName" value="{{item.projectTitle}}">{{item.projectTitle}}</option>													
												</select>
											</div>
										</div>										
										<div class="col-md-2">
										...
										<div class="col-md-2">
											<div class="form-group">
												<select ng-model="referenceby" title="Filter by reference by" ng-options="item.userCompanyName+' - '+item.firstName+' '+item.middleName+' '+item.lastName as item.userCompanyName+' - '+item.firstName+' '+item.middleName+' '+item.lastName for item in getUserNameByUserType" class="form-control">
													<option value="">Reference By</option>
												</select>												
											</div>
										</div>										
										...
										<table class="table no-margin">
											<thead>
												<tr>
													...
													<th class="text-right"><a href="#deleteModal" data-toggle="modal" title="Delete" style="color: red;"><i class="fa fa-trash-o" aria-hidden="true" ng-click="checkRecordSelectForDelete()"></i></a></th>				
												</tr>
											</thead>
											<tbody>
												...
												<tr class="text-center" ng-if="enquiryLoader">
													<td colspan="8" style="font-style: italic;">Loading data... Please Wait...<i class="fa fa-spinner fa-pulse"></i></td>
												</tr>
												<tr ng-repeat="item in (filterResult = (getEnquiries | filter:country | filter:{status:inquirystatus} | filter:project | filter:subcategory | filter:realestate | filter:referenceby))" ng-show="!enquiryLoader" style="cursor:pointer; cursor:hand;" ng-style="(item.status == 'w'  || item.status == 'cw') && {'color':'green'} || item.status == 'h' && {'color':'#f39c12'} || (item.status == 'l' || item.status == 'cl') && {'color':'red'} || item.status == 'o' && {'color':'blue'} || {'color': '#202020'}">
													...
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.firstName}} {{item.lastName}}<br>{{item.userCompanyName}}</td>
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.email}}</td>
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.mobileNumber}}</td>
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.countryName}}</td>
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.referenceBy}}</td>
													<td ng-click="getEnquiryDetail(item.enquiryId)" title="Edit Record" data-toggle="modal" data-target="#editModal" class="text-left">{{item.projectTitle}} {{item.unitName}} {{item.propertyTitle}} {{item.subcategoryTitle}} {{item.realestateTitle}}</td>																	
													<td title="Delete">
									...
									<div class="box-body">
										...
										<div class="row form-group">
											<div class="col-md-3">
												<label>Client<font color="red" size="3">*</font></label>
												<div class="input-group">
													<select id="clientid" name="clientid" ng-model="clientid" ng-options="item.userId as item.userCompanyName+' - '+item.firstName+' '+item.lastName for item in getClientNames" class="form-control">
														<option value="">Client</option>
													</select>
													<span class="input-group-btn">
														<button class="btn btn-info btn-flat" type="button" data-toggle="modal" data-target="#userModal" title="Add New Client"><i class="fa fa-plus"></i></button>
